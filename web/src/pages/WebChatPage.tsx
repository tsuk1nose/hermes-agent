...
import { extractHistoryImages } from "@/lib/web-chat-history";
...
function messageText(value: unknown): string {
  ...
  if (kind === "image" || kind === "input_image" || kind === "image_url") {
    return "[screenshot]";
  }
  return messageText(part.text ?? part.content);
}
...
function parseStoredMessages(rawList: RawMessage[]): ChatMessage[] {
  ...
  for (const m of rawList) {
    const rawContent = messageText(m.content ?? m.text);
    const history = m.role === "user"
      ? extractHistoryImages(rawContent)
      : { attachments: [], content: rawContent };
    ...
    if (m.role === "user" || m.role === "system") {
      parsed.push({
        ...
        role: m.role,
        content,
        attachments: history.attachments.length > 0 ? history.attachments : undefined,
